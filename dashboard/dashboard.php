<?php
	...
	function easyshare() {
		...
		add_option( 'fdbac_txtcolor', '' );
			if ( $key == $fdbac_txtcolor ) {
				echo "$value"; 
			}
		...
		$fdbac_position = get_option('fdbac_txtcolor');
			if ('insert' == $_POST['action_txtcolor']) {
				update_option( 'fdbac_txtcolor', esc_html( $_POST['fdbac_txtcolor'] ) );
			}
...
<div class="fdb-panel">
	<form name="fdbac" method="post" action="<?=$location ?>">
		...
		<!-- ### Add the option "how should it open after clicking" ### -->
		<div class="fdb-option">	
		...
		<!-- ### Add the option "Which color should have the tab" ### -->
		<div class="fdb-option">	
		...
		<!-- ### Add the option "Which color should have the text" ### -->
		<div class="fdb-option">	
			<?php echo '<img class="options-ico"  src="' . plugins_url( 'images/ico_color.svg', __FILE__ ) . '" > '; ?>
			<div class="desc">
				<h4><?php _e('Select text Color','fdbeasyshare') ;?></h4>
				<h5><?php _e('Which color should the text have?','fdbeasyshare') ;?></h5>
			</div>
		
			<input class="my-color-field" name="fdbac_txtcolor" value="<?php echo get_option('fdbac_txtcolor'); ?>" type="text" />
			<input name="action_txtcolor" value="insert" type="hidden" />
		</div>
		
		<!-- ### Add the option "show/hidden Tab Button" ### -->
